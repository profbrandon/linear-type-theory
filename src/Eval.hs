...
eval0 (RecSum t e _ (Inl t' e'))       = App e e'
eval0 (RecSum t _ e (Inr t' e'))       = App e e'
eval0 l@(Lambda s t (App e1 (Var s1)))
  | s1 == s && not (s `elem` getFreeVars e1) = e1
  | otherwise = l
eval0 (Var s)                  = Var s
eval0 Star                     = Star
...
eval0 (Inl t e)                = Inl t (eval0 e)
eval0 (Inr t e)                = Inr t (eval0 e)
eval0 (RecSum t e1 e2 e3)      = RecSum t (eval0 e1) (eval0 e2) (eval0 e3)
...
sub (Inl t e)           p@(_, Left  _) = Inl t (sub e p)
sub (Inl t e)           p@(_, Right _) = Inl (subT t p) (sub e p)
sub (Inr t e)           p@(_, Left  _) = Inr t (sub e p)
sub (Inr t e)           p@(_, Right _) = Inr (subT t p) (sub e p)
sub (RecSum t e1 e2 e3) p@(_, Left  _) = RecSum t (sub e1 p) (sub e2 p) (sub e3 p)
sub (RecSum t e1 e2 e3) p@(_, Right _) = RecSum (subT t p) (sub e1 p) (sub e2 p) (sub e3 p)
sub l@(Lambda s t e)    p@(s', _)
  | s == s'   = l 
  | otherwise = Lambda s (subT t p) (sub e p)
...
subT (TVar s)       (s', Right t)
  | s == s'   = t 
  | otherwise = TVar s
subT u@(Univ _)     _               = u
subT Unit           _               = Unit
subT (Pi s t1 t2)   p@(_ , Left  _) = Pi s (subT t1 p) (subT t2 p)
subT l@(Pi s t1 t2) p@(s', Right t)
  | s == s'   = l
  | otherwise = Pi s (subT t1 p) (subT t2 p)
subT (Prod t1 t2)   p               = Prod (subT t1 p) (subT t2 p)
subT (Sum  t1 t2)   p               = Sum  (subT t1 p) (subT t2 p)
