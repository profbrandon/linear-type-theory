...
arrowEquiv p1@(Pi s t1 t2) p2@(Pi s' t1' t2') =
  if not (s `elem` (getFreeVarsT t2)) && not (s' `elem` (getFreeVarsT t2')) then
    t1 `arrowEquiv` t1' && t2 `arrowEquiv` t2'
  else
    p1 == p2
...
alphaEquiv0 bindings (Var s) (Var s') =
  if (s, s') `elem` bindings then
    (bindings, True)
  else
    case s `lookup` bindings of
      Nothing -> back
      Just ss -> if ss == s' then (bindings, True) else back
  where
    back = 
      case s' `lookup` bindings of
        Nothing  -> (bindings, False)
        Just ss' -> if ss' == s then (bindings, True) else (bindings, False)
        
alphaEquiv0 bindings (RecI t e1 e2) (RecI t' e1' e2') = 
  (b0 `union` b1 `union` b2, a0 && a1 && a2)
  where 
    (b0, a0) = alphaEquivT0 bindings t t'
    (b1, a1) = alphaEquiv0 bindings e1 e1'
    (b2, a2) = alphaEquiv0 bindings e2 e2'

alphaEquiv0 bindings (Pair e1 e2) (Pair e1' e2') =
  (b1 `union` b2, a1 && a2)
  where
    (b1, a1) = alphaEquiv0 bindings e1 e1'
    (b2, a2) = alphaEquiv0 bindings e2 e2'

alphaEquiv0 bindings (RecPair t e1 e2) (RecPair t' e1' e2') =
  (b0 `union` b1 `union` b2, a0 && a1 && a2)
  where 
    (b0, a0) = alphaEquivT0 bindings t t'
    (b1, a1) = alphaEquiv0 bindings e1 e1'
...
alphaEquiv0 bindings (Lambda s t e) (Lambda s' t' e') = 
  (b0 `union` b1, a0 && a1)
  where
    bindings' = (s, s'):bindings
    (b0, a0) = alphaEquivT0 bindings t t'
    (b1, a1)  = alphaEquiv0 bindings' e e'

alphaEquiv0 bindings (App e1 e2) (App e1' e2') = 
  (b1 `union` b2, a1 && a2)
  where
    (b1, a1) = alphaEquiv0 bindings e1 e1'
    (b2, a2) = alphaEquiv0 bindings e2 e2'

alphaEquiv0 bindings (AppT e t) (AppT e' t') = 
  (b0 `union` b1, a0 && a1)
  where
    (b0, a0) = alphaEquivT0 bindings t t'
    (b1, a1) = alphaEquiv0 bindings e e'
...
alphaEquivT0 bindings (Pi s t1 t2) (Pi s' t1' t2') = 
  (b1 `union` b2, a1 && a2)
  where
    bindings' = (s, s'):bindings
...
alphaEquivT0 bindings (Prod t1 t2) (Prod t1' t2')   =
  (b1 `union` b2, a1 && a2)
  where
    (b1, a1) = alphaEquivT0 bindings t1 t1'
    (b2, a2) = alphaEquivT0 bindings t2 t2'
